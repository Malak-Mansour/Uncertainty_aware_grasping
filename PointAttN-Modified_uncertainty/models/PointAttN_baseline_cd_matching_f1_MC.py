...
class cross_transformer(nn.Module):
    def __init__(self, d_model=256, d_model_out=256, nhead=4, dim_feedforward=1024, dropout=0.1):
        super().__init__()
...
class PCT_refine(nn.Module):
    def __init__(self, channel=128, ratio=1):
        ...
        self.sa1 = cross_transformer(channel * 2, 512, dropout=0.1)
        self.sa2 = cross_transformer(512, 512, dropout=0.1)
        self.sa3 = cross_transformer(512, channel * ratio, dropout=0.1)
...
class PCT_encoder_PointAttN(nn.Module):
    def __init__(self, channel=64):
        super(PCT_encoder_PointAttN, self).__init__()
        self.channel = channel
        self.conv1 = nn.Conv1d(3, 64, kernel_size=1)
        self.conv2 = nn.Conv1d(64, channel, kernel_size=1)

        self.sa1 = cross_transformer(channel, channel, dropout=0.1)
        self.sa1_1 = cross_transformer(channel * 2, channel * 2, dropout=0.1)
        self.sa2 = cross_transformer(channel * 2, channel * 2, dropout=0.1)
        self.sa2_1 = cross_transformer(channel * 4, channel * 4, dropout=0.1)
        self.sa3 = cross_transformer(channel * 4, channel * 4, dropout=0.1)
        self.sa3_1 = cross_transformer(channel * 8, channel * 8, dropout=0.1)

        self.relu = nn.GELU()

        self.sa0_d = cross_transformer(channel * 8, channel * 8, dropout=0.1)
        self.sa1_d = cross_transformer(channel * 8, channel * 8, dropout=0.1)
        self.sa2_d = cross_transformer(channel * 8, channel * 8, dropout=0.1)
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = PCT_encoder_PointAttN()
