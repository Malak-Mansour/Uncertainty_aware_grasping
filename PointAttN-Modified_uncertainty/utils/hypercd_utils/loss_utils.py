...
from .hyp_utils import fps_subsample
...
def get_loss1(pcds_pred, partial, gt, sqrt=True):
    ...
    # Subsample the predicted point clouds to match gt's size
    #P3_subsampled = fps_subsample(P3, gt.shape[1])
    P2_subsampled = fps_subsample(P2, gt.shape[1])
    P1_subsampled = fps_subsample(P1, gt.shape[1])
    gt_c = fps_subsample(gt, Pc.shape[1])
    
    # Calculate CD with the same gt and subsampled predictions
    cdc = calc_cd_like_hyperV2(Pc, gt_c)
    cd1 = calc_cd_like_hyperV2(P1_subsampled, gt)
    cd2 = calc_cd_like_hyperV2(P2_subsampled, gt)
    # cd3 = CD(P3, gt)
